...
class ScreenSizeWithValue<T extends Enum, V extends Object?>
  ...
  /// Overrides the `FlutterView` used to read physical metrics in tests.
...
class _ScreenSizeWithValueState<T extends Enum, V extends Object?>
  ...
  Widget build(BuildContext context) {
    ...
    return _ScreenSizeProvider<T>(
      screenSize: newData.screenSize,
      child: ScreenSizeModelWithValue<T, V>(
        data: newData,
        child: widget.child,
      ),
    );
...
/// Private InheritedWidget for type-erased breakpoint lookups.
/// Allows breakpointOf<K>() to work without knowing the value type V.
class _ScreenSizeProvider<K extends Enum> extends InheritedWidget {
  ...
  @override
  bool updateShouldNotify(_ScreenSizeProvider<K> oldWidget) =>
      screenSize != oldWidget.screenSize;
}

/// Inherited model that exposes screen metrics and the resolved value.
class ScreenSizeModelWithValue<T extends Enum, V extends Object?>
  ...
  /// Returns the current screen data and responsive value from the nearest model.
...
''');
  ...
  /// Returns the current responsive value from the nearest model.
...
''');
  ...
  /// Returns the currently resolved responsive breakpoint enum.
  ///
  /// This accessor only rebuilds dependents when the resolved breakpoint
  /// changes and does not require the responsive value type.
  static K breakpointOf<K extends Enum>(
    BuildContext context,
  ) {
    final provider =
        context.dependOnInheritedWidgetOfExactType<_ScreenSizeProvider<K>>();
    if (provider == null) {
      throw FlutterError('''
ScreenSizeWithValue<$K, ?> not found. Please ensure that the app is wrapped in
...
''');
  ...
  /// Returns the currently resolved responsive breakpoint enum.
  @Deprecated(
    'Use breakpointOf instead. screenSizeOf returns a breakpoint enum, not raw screen dimensions.',
  )
  static K screenSizeOf<K extends Enum>(
    BuildContext context,
  ) {
    return breakpointOf<K>(context);
  }
...
/// Immutable screen metrics with the resolved responsive value.
@immutable
