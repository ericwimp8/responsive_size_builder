...
class ScreenSize<T extends Enum> extends StatefulWidget {
  ...
  /// The breakpoint definitions used to resolve the current screen size.
  final BaseBreakpoints<T> breakpoints;

  /// Optional view used to read metrics in tests.
...
/// Inherited model that exposes screen-size data to descendants.
class ScreenSizeModel<T extends Enum> extends InheritedModel<ScreenSizeAspect> {
  ...
  /// Returns the current screen-size model for [context].
...
''');
  ...
  /// changes, not on every raw screen-size update.
...
/// Immutable screen metrics and breakpoint resolution data.
@immutable
class ScreenSizeModelData<K extends Enum> {
  ...
  /// The breakpoint threshold matched for the current screen size.
  final double currentBreakpoint;

  /// The resolved breakpoint enum for the current screen.
  final K screenSize;
...
/// Dependency aspects exposed by [ScreenSizeModel].
