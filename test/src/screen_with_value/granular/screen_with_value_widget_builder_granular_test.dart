import 'package:flutter/material.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:responsive_size_builder/responsive_size_builder.dart';

import '../../../harness/harness.dart';

void main() {
  group('ScreenWithValueWidgetBuilderGranular', () {
    test('requires at least one builder', () {
      expect(
        ScreenWithValueWidgetBuilderGranular<String>.new,
        throwsAssertionError,
      );
    });

    testWidgets(
      'resolves builder from screen width and exposes screen data with responsive value',
      (tester) async {
        ScreenSizeModelDataWithValue<LayoutSizeGranular, String>? resolvedData;

        await pumpHarnessApp(
          tester: tester,
          addTearDown: addTearDown,
          customSize: const Size(1024, 700),
          withScaffold: false,
          child: ScreenSizeWithValue<LayoutSizeGranular, String>(
            breakpoints: BreakpointsGranular.defaultBreakpoints,
            valueProvider: ResponsiveValueGranular<String>(
              standardLarge: 'fromProvider',
              compactNormal: 'providerFallback',
            ),
            child: ScreenWithValueWidgetBuilderGranular<String>(
              standardLarge: (_, data) {
                resolvedData = data;
                return const SizedBox(key: Key('standard-large'));
              },
              compactNormal: (_, __) =>
                  const SizedBox(key: Key('compact-normal')),
            ),
          ),
        );

        expect(find.byKey(const Key('standard-large')), findsOneWidget);
        expect(find.byKey(const Key('compact-normal')), findsNothing);
        expect(resolvedData, isNotNull);
        expect(resolvedData!.screenSize, LayoutSizeGranular.standardLarge);
        expect(resolvedData!.responsiveValue, 'fromProvider');
        expect(resolvedData!.logicalScreenWidth, 1024);
        expect(resolvedData!.logicalScreenHeight, 700);
      },
    );

    testWidgets('uses custom breakpoints for handler resolution', (
      tester,
    ) async {
      const breakpoints = BreakpointsGranular(
        jumboExtraLarge: 1000,
        jumboLarge: 900,
        jumboNormal: 800,
        jumboSmall: 700,
        standardExtraLarge: 600,
        standardLarge: 500,
        standardNormal: 400,
        standardSmall: 300,
        compactExtraLarge: 200,
        compactLarge: 150,
        compactNormal: 100,
        compactSmall: 50,
      );

      await pumpHarnessApp(
        tester: tester,
        addTearDown: addTearDown,
        customSize: const Size(450, 700),
        withScaffold: false,
        child: ScreenSizeWithValue<LayoutSizeGranular, String>(
          breakpoints: breakpoints,
          valueProvider: ResponsiveValueGranular<String>(
            breakpoints: breakpoints,
            standardNormal: 'providerStandardNormal',
            compactExtraLarge: 'providerCompactExtraLarge',
          ),
          child: ScreenWithValueWidgetBuilderGranular<String>(
            breakpoints: breakpoints,
            standardNormal: (_, data) => SizedBox(
              key: const Key('standard-normal'),
              child: Text(data.responsiveValue),
            ),
            compactExtraLarge: (_, __) =>
                const SizedBox(key: Key('compact-extra-large')),
          ),
        ),
      );

      expect(find.byKey(const Key('standard-normal')), findsOneWidget);
      expect(find.text('providerStandardNormal'), findsOneWidget);
      expect(find.byKey(const Key('compact-extra-large')), findsNothing);
    });

    testWidgets('rebuilds handler when widget builders change', (tester) async {
      await pumpHarnessApp(
        tester: tester,
        addTearDown: addTearDown,
        customSize: const Size(1024, 700),
        withScaffold: false,
        child: ScreenSizeWithValue<LayoutSizeGranular, String>(
          breakpoints: BreakpointsGranular.defaultBreakpoints,
          valueProvider: ResponsiveValueGranular<String>(
            standardLarge: 'providerValue',
          ),
          child: ScreenWithValueWidgetBuilderGranular<String>(
            standardLarge: (_, __) => const SizedBox(key: Key('first-value')),
          ),
        ),
      );

      expect(find.byKey(const Key('first-value')), findsOneWidget);

      await tester.pumpWidget(
        buildHarnessApp(
          withScaffold: false,
          child: ScreenSizeWithValue<LayoutSizeGranular, String>(
            breakpoints: BreakpointsGranular.defaultBreakpoints,
            valueProvider: ResponsiveValueGranular<String>(
              standardLarge: 'providerValue',
            ),
            child: ScreenWithValueWidgetBuilderGranular<String>(
              standardLarge: (_, __) =>
                  const SizedBox(key: Key('updated-value')),
            ),
          ),
        ),
      );
      await tester.pump();

      expect(find.byKey(const Key('updated-value')), findsOneWidget);
      expect(find.byKey(const Key('first-value')), findsNothing);
    });

    testWidgets('wraps resolved child in AnimatedSwitcher when enabled', (
      tester,
    ) async {
      await pumpHarnessApp(
        tester: tester,
        addTearDown: addTearDown,
        customSize: const Size(420, 700),
        withScaffold: false,
        settle: false,
        child: ScreenSizeWithValue<LayoutSizeGranular, String>(
          breakpoints: BreakpointsGranular.defaultBreakpoints,
          valueProvider: ResponsiveValueGranular<String>(
            compactNormal: 'providerValue',
          ),
          child: ScreenWithValueWidgetBuilderGranular<String>(
            compactNormal: (_, __) =>
                const SizedBox(key: Key('animated-child')),
            animateChange: true,
          ),
        ),
      );

      expect(find.byType(AnimatedSwitcher), findsOneWidget);
      expect(find.byKey(const Key('animated-child')), findsOneWidget);
    });
  });
}
